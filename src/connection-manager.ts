...
export class ConnectionManager {
  ...
  private setupRuntimeReconnection(): void {
    ...
    if (client.socket) {
      ...
      // activity. ConnectionManager intentionally avoids sending its own
      // websocket ping frames here because the DingTalk stream session is only
      // considered ready after REGISTERED, and active probes on pre-registered
      // sockets were observed to correlate with premature 1006 closes.
